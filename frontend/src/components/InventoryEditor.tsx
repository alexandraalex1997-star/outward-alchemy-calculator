    ...
    >
      <div className="inventory-manager-shell">
        ...
        <div className="inventory-editor inventory-manager-section">
          <div className="inventory-editor-workspace">
            <form className="quick-add-row control-strip inventory-subsection" onSubmit={(event) => void onQuickAdd(event)}>
            ...
            <div className="toolbar-row control-strip toolbar-strip inventory-subsection">
            ...
            {filteredCatalogRows.length ? (
              <div className="inventory-table-section inventory-subsection">
              ...
              <div className="empty-state inventory-subsection">No ingredients match the current search and category filters.</div>
            )}
          </div>
        </div>

        <div className="inventory-foot-stats inventory-manager-section">
          <StatCard className="inventory-foot-stat" label="Categories shown" value={selectedCategories.length || categories.length} />
          <StatCard className="inventory-foot-stat" label="Visible now" value={filteredCatalogRows.length} />
          <StatCard className="inventory-foot-stat" label="Selected total" value={inventory?.total_quantity ?? 0} />
          <StatCard className="inventory-foot-stat" label="Unique selected" value={inventory?.unique_items ?? 0} />
        </div>
      </div>
    </Panel>
  );
